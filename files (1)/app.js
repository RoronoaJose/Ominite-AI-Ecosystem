...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- CANVAS: PULSATING PARTICLE-SPHERE BACKGROUND ---
  // A cloud of small dash-shaped particles arranged on a sphere, gently
  // rotating. Each particle breathes in and out along its own radial line
  // on its own staggered heartbeat timing, so the pulse flows through the
  // cluster one piece at a time rather than the whole ball swelling at once.
  function initCanvas() {
    ...
    // Cluster center eases toward the cursor; holds its position when the
    // cursor hasn't moved yet or has left the window, so it never stops.
    ...
    // --- Build a sphere of particles (Fibonacci sphere = even spacing) ---
    const PARTICLE_COUNT = 300;
    const GOLDEN_ANGLE = Math.PI * (3 - Math.sqrt(5));
    const BEAT_DURATION = 3400; // ms per full heartbeat cycle — slow, calm
    const WAVE_SPREAD = 0.7; // fraction of the cycle used to stagger the wave across particles
    const particles = [];
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      particles.push({
        ...
        // Stagger each particle's beat by its position in the sequence plus
        // a little jitter, so the pulse ripples through the cluster instead
        // of every particle swelling in unison.
        beatOffset: (i / PARTICLE_COUNT) * BEAT_DURATION * WAVE_SPREAD + Math.random() * 260
      });
    }

    // --- Heartbeat curve: gentle single soft swell (slow, subtle) ---
    function heartbeatPulse(t) {
      const d = (t - 0.16) / 0.22;
      return Math.exp(-d * d * 3.2);
    }
    ...
    function animate(time) {
      ...
      if (state.activeView === 'landing') {
        // Ease cluster origin toward the cursor
        origin.x += (mouse.x - origin.x) * 0.05;
        origin.y += (mouse.y - origin.y) * 0.05;

        const sphereRadius = 300; // fixed — the ball itself doesn't swell as a whole
        const rotY = time * 0.00009;
        const rotX = Math.sin(time * 0.00006) * 0.32;
        const cosY = Math.cos(rotY), sinY = Math.sin(rotY);
        const cosX = Math.cos(rotX), sinX = Math.sin(rotX);

        // Sort back-to-front so nearer particles draw on top
        ...
        projected.forEach(({ p, x, y, z }) => {
          const depth = (z + 1) / 2; // 0 (far) .. 1 (near)

          // Each particle runs its own heartbeat cycle, offset from the others,
          // so the "breathing" travels through the cluster rather than the
          // whole sphere expanding and collapsing together.
          const localCycle = ((time + p.beatOffset) % BEAT_DURATION) / BEAT_DURATION;
          const pulse = heartbeatPulse(localCycle);

          // Particle drifts slightly in/out along its own radial line — a
          // subtle individual breathing motion, not a uniform ball-wide scale.
          const r = sphereRadius + pulse * 20;
          const px = origin.x + x * r;
          const py = origin.y + y * r;

          const twinkle = 0.85 + Math.sin(time * p.twinkleSpeed + p.twinklePhase) * 0.15;
          const alpha = (0.16 + depth * 0.58) * (0.8 + pulse * 0.35) * twinkle;
          const size = p.size * (0.6 + depth * 0.8) * (0.82 + pulse * 0.35);
          ...
          ctx.fillStyle = depth > 0.82
            ...
            : `rgba(118, 137, 247, ${alpha})`;
          if (ctx.roundRect) {
            ctx.beginPath();
            ctx.roundRect(-len / 2, -size / 2, len, size, size / 2);
            ctx.fill();
          } else {
            ctx.beginPath();
        ...
        // Dashboard: quiet ambient vignette, no active pulse
        const grad = ctx.createRadialGradient(width / 2, height * 0.15, 10, width / 2, height * 0.15, width * 0.5);
        grad.addColorStop(0, 'rgba(18, 18, 20, 0.5)');
